...
from ctypes import RTLD_GLOBAL, CDLL, c_char_p, c_void_p, byref, POINTER, c_int, c_double
...
class sparta:
    def __init__(self, name="", cmdargs=None):
        ...
        try:
            if not name:
                self.lib = CDLL("libsparta.so", RTLD_GLOBAL)
            else:
                self.lib = CDLL("libsparta_%s.so" % name, RTLD_GLOBAL)
        except:
    ...
    def read_file(self, file_path):
        log.info(file_path)
        log.info(file_path)
        self.lib.sparta_file(self.spa, file_path)
